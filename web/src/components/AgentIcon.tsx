// AgentIcon:agent 展示图标(行内)。claude-code / codex 各用品牌 logo SVG
// (web/src/logo/claudecode-color.svg、web/src/logo/codex-cli.svg),未知 agent 回退方块符号。
// 统一替代 agentMeta.icon 的 emoji。
//
// 用 ReactNode 而非 string:antd Select/Tabs 的 label 接受 ReactNode,行内渲染也能对齐基线。
// SVG 内联(非 <img>):尺寸随 font-size 缩放(1em),与文字同行高度对齐。
// 品牌身份色走 token(--agent-claude / --agent-codex,design.md:与 accent 平级,不进 category),
// 浅深两套各定 oklch,不硬编码品牌 hex。
...
// Codex CLI 品牌 logo path(源自 web/src/logo/codex-cli.svg,fill 用 --agent-codex token)。
  ...
  if (id === 'claude-code') {
    return (
      ...
      >
        ...
        <path d={CLAUDE_CODE_PATH} fill="var(--agent-claude)" fillRule="nonzero" />
      </svg>
    )
  }
  if (id === 'codex') {
    return (
      ...
      >
        ...
        <path d={CODEX_CLI_PATH} fill="var(--agent-codex)" fillRule="evenodd" />
      </svg>
    )
  }
