    ...
    <div data-testid="detector-chips" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
        <button type="button" onClick={() => onSelect?.(undefined)} aria-pressed={!selectedId}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 16, cursor: 'pointer', fontSize: 'var(--fs-sm)', background: !selectedId ? 'var(--color-accent-soft)' : 'var(--color-paper-2)', border: `1px solid ${!selectedId ? 'var(--color-accent)' : 'var(--color-rule)'}`, color: 'var(--color-ink)' }}>
        ...
        {detectors.map((d) => (
          <button key={d.id} type="button" onClick={() => onSelect?.(selectedId === d.id ? undefined : d.id)} aria-pressed={selectedId === d.id} data-testid="detector-chip"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 16, cursor: 'pointer', fontSize: 'var(--fs-sm)', background: selectedId === d.id ? 'var(--color-accent-soft)' : 'var(--color-paper-2)', border: `1px solid ${selectedId === d.id ? 'var(--color-accent)' : 'var(--color-rule)'}`, color: 'var(--color-ink)' }}>
            <DetectorDot status={statusBadge(d)} />
            <span>{detectorName(d)}</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-xs)', color: 'var(--color-muted)' }}>{ruleCountLabel(d, t)}</span>
          </button>
        ))}
      </div>
      {selected ? (
        <Card size="small" style={{ background: 'var(--color-surface)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div>
              <Typography.Text type="secondary" style={{ fontSize: 'var(--fs-sm)' }}>{t('detector.status')}</Typography.Text>
              <div style={{ marginTop: 4 }}>
                ...
                {!selected.enabled ? null : !selected.available && selected.reason ? <Typography.Text type="secondary" style={{ fontSize: 'var(--fs-xs)', marginLeft: 8 }}>{selected.reason}</Typography.Text> : null}
              </div>
            </div>
            <div>
              <Typography.Text type="secondary" style={{ fontSize: 'var(--fs-sm)' }}>{t('detector.engines')}</Typography.Text>
              <div style={{ marginTop: 4 }}>
                {(selected.engines ?? []).map((e) => (
                  <div key={e.name} style={{ fontSize: 'var(--fs-sm)' }}>
                    <DetectorDot status={!e.enabled ? 'default' : e.available ? 'success' : 'error'} />
                    <span style={{ color: 'var(--color-ink)', marginLeft: 4 }}>{e.name}</span>
                    <Typography.Text type="secondary" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-xs)', marginLeft: 8 }}>{e.kind}</Typography.Text>
                    {e.enabled && !e.available && e.reason ? <Typography.Text type="secondary" style={{ fontSize: 'var(--fs-xs)', marginLeft: 8 }}>{e.reason}</Typography.Text> : null}
                  </div>
                ))}
              </div>
            </div>
            {selected.covers && selected.covers.length > 0 ? (
              <div>
                <Typography.Text type="secondary" style={{ fontSize: 'var(--fs-sm)' }}>{t('detector.covers')}</Typography.Text>
