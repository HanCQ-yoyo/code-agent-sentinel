...
func newScanCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "scan",
		Short: "Run one-off scan and write history (no HTTP server)",
	...
	cmd.Flags().StringVar(&detectorsFlag, "detectors", "", "Run only specified detectors (comma-separated IDs; empty=all)")
	cmd.Flags().StringVar(&agentFlag, "agent", "", "Target code agent ID (empty=first agent)")
	return cmd
}
...
func runScanCmd(cmd *cobra.Command, cfgPath, detectorsFlag, agentFlag string) error {
	...
	out := cmd.OutOrStdout()
	fmt.Fprintf(out, "Scan completed: %d findings, duration %s\n", len(res.Findings), res.Duration.Round(time.Millisecond))
	...
	for _, d := range res.Detectors {
		if !d.Available {
			fmt.Fprintf(out, "Detector %s unavailable: %s\n", d.ID, d.Reason)
		}
	}
	return nil
}
