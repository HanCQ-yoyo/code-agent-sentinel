...
func parseHooksFromData(data []byte, path string, scope Scope) []Asset {
	...
	if err := json.Unmarshal(data, &rs); err == nil && rs.Hooks != nil {
		return hooksToAssets(rs.Hooks, path, scope)
	}
	...
	return hooksToAssets(hooks, path, scope)
}
...
func hooksToAssets(hooks map[string][]rawHookEntry, path string, scope Scope) []Asset {
	var out []Asset
