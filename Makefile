...
# ---- 本地开发 ----
build: web
	go build -o bin/code-agent-sentinel ./cmd/sentinel
...
#   若 dist/code-agent-sentinel-<os>-<arch>-<commit>.{tar.gz,zip} 已存在则跳过。
define build-one
	@GOOS="$(1)"; GOARCH="$(2)"; \
	if [ "$$GOOS" = "windows" ]; then EXT=".exe"; else EXT=""; fi; \
	BASE="code-agent-sentinel-$$GOOS-$$GOARCH-$(COMMIT)$$EXT"; \
	if [ "$$GOOS" = "windows" ]; then ARCHIVE="$(DIST)/$${BASE%.exe}.zip"; \
	else ARCHIVE="$(DIST)/$${BASE%.exe}.tar.gz"; fi; \
	if [ -f "$$ARCHIVE" ]; then \
		echo "  $$GOOS/$$GOARCH [$(COMMIT)] 已存在,跳过"; \
	else \
		echo "  $$GOOS/$$GOARCH -> $(DIST)/code-agent-sentinel-$$GOOS-$$GOARCH$$EXT"; \
		GOOS=$$GOOS GOARCH=$$GOARCH CGO_ENABLED=0 \
			go build -ldflags "-s -w" \
			-o "$(DIST)/code-agent-sentinel-$$GOOS-$$GOARCH$$EXT" ./cmd/sentinel; \
		cd $(DIST); \
		if [ "$$GOOS" = "windows" ]; then \
			zip "$${BASE%.exe}.zip" "code-agent-sentinel-$$GOOS-$$GOARCH$$EXT" && rm "code-agent-sentinel-$$GOOS-$$GOARCH$$EXT"; \
		else \
			tar czf "$${BASE%.exe}.tar.gz" "code-agent-sentinel-$$GOOS-$$GOARCH$$EXT" && rm "code-agent-sentinel-$$GOOS-$$GOARCH$$EXT"; \
		fi; \
	fi
endef
